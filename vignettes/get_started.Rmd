...
tcga_brca_test %>%
  plot_SFS() +
  geom_mutations(drivers = "BRCA", shape = "variant_classification")

tcga_brca_test %>%
  plot_SFS() +
  geom_mutations(drivers = "BRCA", shape = "impact")
...
plot(Mf_1f, from = 0.05, to = 0.5, scale = FALSE)
  # geom_smooth() +
...
neutralitytest_res <- tcga_brca_test %>%
  group_by(sample_id) %>%
  nest() %>%
  deframe() %>%
  map("VAF") %>%
...
plot_SFS(snvs_AMLRO, geom = "bar", show.legend = FALSE) +
  ...
  geom_mutations(drivers = "LAML", color = "black")
```

# Fit partial neutral models
...
plot_SFS(tcga_brca_test, geom = "bar", show.legend = FALSE, color = "white", alpha = 0.9) +
  ...
  scale_color_pnw() +
  scale_fill_pnw() +
  geom_mutations(drivers = "BRCA", color = "black") +
  geom_line(aes(x, y), data = exp, color = "black", size = 1, linetype = "dashed") +
  geom_line(aes(x, y), data = exp |> filter(neutr), color = "black", size = 1)
```

```{r}
fake_VAFs <- generate_neutral_snvs(mut_rate = 10, sample_below = 0.15)
fake_VAFs

plot_SFS(fake_VAFs, geom = "bar") +
  xlim(0, 1)
###
...
plot(Mf_1f, scale = FALSE, from = 0) +
  geom_lm_models(models)

exp <- models |> 
  slice(1) |> 
  expand_grid(x = seq(0.05, 1, by = 0.01)) |> 
  filter(x >= from, x <= to) |> 
  mutate(
    y = (a/-90)/x^2 + 0
  )

plot_SFS(fake_VAFs, geom = "bar", show.legend = FALSE, alpha = 0.4, color = "white") +
  scale_color_pnw() +
  scale_fill_pnw() +
  geom_line(aes(x, y), data = exp, color = "black")
```

```{r}
tibble(
    mut_rate = c(1, 2, 4, 6, 10, 20),
    a = c(-90, -180, -360, -540, -900, -1800)
  ) |> 
  ggplot(aes(mut_rate, a)) +
  geom_point() +
  theme_minimal()
```
