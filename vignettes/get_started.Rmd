...
```{r}
get_models(cd, name = "powerlaw_fixed")
```

The active model is returned if the `name` argument is left empty:
...
All evolutionary parameters can be obtained with `get_evolutionary_parameters()` function, which can be run on the cevodata object (with the correct models fitted), or on the model objects directly.
